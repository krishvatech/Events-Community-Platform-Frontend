...
export default function AdminSettings() {
  ...
  const [integrityPromptDismissed, setIntegrityPromptDismissed] = React.useState(false);
    ...
    <Box sx={{ minHeight: "100vh", bgcolor: "background.default" }}>
                  ...
                  <SectionCard title="E-Mail" action={
                    ...
                    <Stack spacing={1}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        ...
                        {/* Right: Metadata */}
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          {renderVisibilityIcon(profile.links?.contact?.main_email?.visibility || "contacts")}
      ...
      <Dialog open={expOpen} onClose={() => { setExpOpen(false); setEditExpId(null); }} fullWidth maxWidth="sm" fullScreen={isMobile}>
        ...
        <DialogContent dividers>
          ...
          <TextField select label="Work schedule (optional)" value={expForm.work_schedule} onChange={(e) => setExpForm((f) => ({ ...f, work_schedule: e.target.value }))} fullWidth sx={{ mb: 2 }} error={!!expReqErrors.work_schedule} helperText={expReqErrors.work_schedule || ""}>
            <MenuItem value="full_time">Full-time</MenuItem><MenuItem value="part_time">Part-time</MenuItem>
          </TextField>
          <TextField select fullWidth label="Career stage (optional)" value={expForm.career_stage} onChange={(e) => setExpForm((f) => ({ ...f, career_stage: e.target.value }))} sx={{ mb: 1 }} error={!!expReqErrors.career_stage} helperText={expReqErrors.career_stage || ""}>
          ...
          <TextField select label="Work arrangement (optional)" value={expForm.work_arrangement} onChange={(e) => setExpForm((f) => ({ ...f, work_arrangement: e.target.value }))} fullWidth sx={{ mb: 2 }} error={!!expReqErrors.work_arrangement} helperText={expReqErrors.work_arrangement || ""}>
